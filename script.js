...
function clickme () 
{
    ...
    // let p_reason = document.getElementById('p_reason');
    ...
    let dates = today.getFullYear()+'0'+(today.getMonth()+1)+today.getDate()+`${localStorage.getItem('patientNo')}`;
    r_date = dates;
...
function printme () 
{
    ...
    {
    doc.fromHTML(`<html><head><title>${title}</title></head><body>` + document.getElementById('print').innerHTML + `${checked}` +`</body></html>`);
